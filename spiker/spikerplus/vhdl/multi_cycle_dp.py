...
class MultiCycleDP(VHDLblock):
    def __init__(self, n_cycles=10, debug=False, debug_list=[]):
        ...
        self.cmp = Cmp(
            bitwidth=self.cycles_cnt_bitwidth, cmp_type="eq", signal_type="std_logic"
        )
    ...
    def vhdl(self, debug=False, debug_list=None):
        ...
        self.entity.port.add(
            name="cycles_cnt_en", direction="in", port_type="std_logic"
        )
        self.entity.port.add(
            name="cycles_cnt_rst_n", direction="in", port_type="std_logic"
        )

        # Output
        self.entity.port.add(name="stop", direction="out", port_type="std_logic")

        # Signals
        self.architecture.signal.add(
            name="cycles_cnt",
            signal_type="std_logic_vector(cycles_cnt_bitwidth-1 downto 0)",
        )
        ...
        self.architecture.instances["cycles_counter"].g_map.add(
            "N", "cycles_cnt_bitwidth"
        )
        del self.architecture.instances["cycles_counter"].g_map["rst_value"]
        ...
        self.architecture.instances["cycles_counter"].p_map.add("cnt_out", "cycles_cnt")

        # Cycles cmp
        self.architecture.instances.add(self.cmp, "cycles_cmp")
        self.architecture.instances["cycles_cmp"].generic_map()
        self.architecture.instances["cycles_cmp"].g_map.add("N", "cycles_cnt_bitwidth")
        self.architecture.instances["cycles_cmp"].port_map()
        self.architecture.instances["cycles_cmp"].p_map.add("in0", "cycles_cnt")
        self.architecture.instances["cycles_cmp"].p_map.add(
            "in1", "std_logic_vector(to_unsigned(n_cycles + 1, cycles_cnt_bitwidth))"
        )
        self.architecture.instances["cycles_cmp"].p_map.add("cmp_out", "stop")
...
class MultiCycleDP_tb(Testbench):
    ...
    def vhdl(self, clock_period=20, file_output=False):
        ...
        self.architecture.processes["cycles_cnt_en_gen"].bodyHeader.add(
            "cycles_cnt_en <= '0';"
        )
        ...
        self.architecture.processes["cycles_cnt_en_gen"].bodyHeader.add(
            "wait for " + str(3 * clock_period * self.n_cycles * 2) + " ns;"
        )
        ...
        self.architecture.processes["cycles_cnt_rst_n_gen"].sensitivity_list.add("clk")
        self.architecture.processes["cycles_cnt_rst_n_gen"].sensitivity_list.add("stop")
        ...
        self.architecture.processes["cycles_cnt_rst_n_gen"].if_list.add()
        self.architecture.processes["cycles_cnt_rst_n_gen"].if_list[
            0
        ]._if_.conditions.add("clk'event")
        self.architecture.processes["cycles_cnt_rst_n_gen"].if_list[
            0
        ]._if_.conditions.add("clk = '1'", "and")
        self.architecture.processes["cycles_cnt_rst_n_gen"].if_list[0]._if_.body.add(
            cycles_stop_if
        )

        self.architecture.processes["cycles_cnt_rst_n_gen"].final_wait = False
