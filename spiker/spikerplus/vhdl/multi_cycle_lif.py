...
class MultiCycleLIF(VHDLblock):
    ...
    def vhdl(self, debug=False, debug_list=None):
        ...
        self.entity.port.add(name="mc_ready", direction="out", port_type="std_logic")

        self.entity.port.add(name="out_spike", direction="out", port_type="std_logic")

        self.entity.port.add(name="start_all", direction="out", port_type="std_logic")

        # Signals
        self.architecture.signal.add(name="start_all_sig", signal_type="std_logic")

        self.architecture.signal.add(name="restart", signal_type="std_logic")

        self.architecture.signal.add(name="all_ready", signal_type="std_logic")

        # Components
        self.architecture.component.add(self.multi_cycle)
        self.architecture.component.add(self.multi_input_lif)

        self.architecture.bodyCodeHeader.add("start_all <= start_all_sig;")

        # Multi-cycle
        self.architecture.instances.add(self.multi_cycle, "mc")
        ...
        self.architecture.instances["mc"].p_map.add("start_all", "start_all_sig")

        # Multi-input neuron
        self.architecture.instances.add(self.multi_input_lif, "multi_input_neuron")
...
class MultiCycleLIF_tb(Testbench):
    ...
    def vhdl(
        ...
        self.architecture.processes["exc_weight_gen"].bodyHeader.add(
            "exc_weight <= to_signed(550000000, exc_weight'length);"
        )

        # inh_weight
        self.architecture.processes["inh_weight_gen"].bodyHeader.add(
            "inh_weight <= to_signed(-400000000, inh_weight'length);"
        )

        # v_reset
        self.architecture.processes["v_reset_gen"].bodyHeader.add(
            "v_reset <= to_signed(1000000, v_reset'length);"
        )

        # v_th
        self.architecture.processes["v_th_gen"].bodyHeader.add(
            "v_th <= to_signed(1500000000, v_th'length);"
        )
        ...
        self.architecture.processes["start_gen"].if_list[0]._if_.body.add(mc_ready_if)
        ...
        self.architecture.bodyCodeHeader.add("exc_spikes_rd_en <= start_all;")
        del self.architecture.processes["inh_spikes_rd_en_gen"]
        self.architecture.bodyCodeHeader.add("inh_spikes_rd_en <= start_all;")
