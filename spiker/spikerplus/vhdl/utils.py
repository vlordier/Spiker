...
def ceil_pow2(x: int) -> int:
    ...
    while go:
        if x // 2**power == 0 or (x // 2**power == 1 and x % 2**power == 0):
            go = False
...
def int_to_hex(value: int, width: int = 8, fill: int = 0) -> str:
    ...
    return "{0:{fill}{width}{base}}".format(value, fill=fill, width=width, base="x")
...
def obj_types(obj: object) -> list[str]:
    """Returns the list of types of an object, including those of the bases."""
    types: list[str] = []
...
def generate_spikes(filename: str, n_spikes: int, n_cycles: int) -> None:
