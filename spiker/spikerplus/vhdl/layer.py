...
from .utils import ceil_pow2, fixed_point_array, int_to_bin, int_to_hex
...
class Layer(VHDLblock):
    def __init__(
        self,
        label="",
        w_exc=np.array([[0.2, 0.3]]),
        w_inh=np.array([[-0.1, -0.2]]),
        v_th=np.array([8]),
        v_reset=np.array([2]),
        bitwidth=16,
        fp_decimals=0,
        ...
        self.w_exc = w_exc
        self.w_inh = w_inh
        self.v_th = fixed_point_array(v_th, bitwidth, fp_decimals)
        ...
        if self.lif_neuron.reset == "fixed":
            self.v_reset = fixed_point_array(v_reset, bitwidth, fp_decimals)
    ...
    def vhdl(self, debug=False, debug_list=None):
        ...
        self.entity.port.add(
            name="out_spikes",
            direction="out",
            port_type="std_logic_vector(" + str(self.n_neurons - 1) + " downto 0)",
        )
        ...
        for i in range(self.n_neurons):
            self.architecture.constant.add(
                name="v_th_" + int_to_hex(i, hex_width),
                const_type="signed(neuron_bit_width-1 downto 0)",
                value='"' + int_to_bin(self.v_th[i], self.bitwidth) + '"',
            )

            if self.lif_neuron.reset == "fixed":
                self.architecture.constant.add(
                    name="v_reset_" + int_to_hex(i, hex_width),
                    const_type="signed(neuron_bit_width-1 downto 0)",
                    value='"' + int_to_bin(self.v_reset[i], self.bitwidth) + '"',
                )

        # Signals
        self.architecture.signal.add(name="start_neurons", signal_type="std_logic")

        self.architecture.signal.add(name="neurons_restart", signal_type="std_logic")

        self.architecture.signal.add(name="neurons_ready", signal_type="std_logic")

        self.architecture.signal.add(name="exc", signal_type="std_logic")

        self.architecture.signal.add(name="inh", signal_type="std_logic")

        self.architecture.signal.add(name="exc", signal_type="std_logic")

        self.architecture.signal.add(name="inh", signal_type="std_logic")

        self.architecture.signal.add(name="exc_spike", signal_type="std_logic")

        self.architecture.signal.add(name="inh_spike", signal_type="std_logic")
        ...
        self.architecture.signal.add(
            ...
            signal_type="std_logic_vector(" + str(self.n_neurons - 1) + " downto 0)",
        )

        self.architecture.signal.add(name="out_sample", signal_type="std_logic")

        for i in range(self.n_neurons):
            hex_index = int_to_hex(i, hex_width)

            self.architecture.signal.add(
                name="neuron_ready_" + hex_index, signal_type="std_logic"
            )

            if self.w_inh_bw < self.bitwidth:
                self.architecture.signal.add(
                    name="inh_weight_" + int_to_hex(i, hex_width),
                    signal_type="std_logic_vector(inh_weights_bit_width-1 downto 0)",
                )
            elif self.w_inh_bw == self.bitwidth:
                self.architecture.signal.add(
                    name="inh_weight_" + int_to_hex(i, hex_width),
                    signal_type="std_logic_vector(neuron_bit_width-1 downto 0)",
                )
            else:
                raise ValueError(
            ...
            if self.w_exc_bw < self.bitwidth:
                self.architecture.signal.add(
                    name="exc_weight_" + int_to_hex(i, hex_width),
                    signal_type="std_logic_vector(exc_weights_bit_width-1 downto 0)",
                )
        ...
        for i in range(self.n_neurons):
            ...
            neurons_ready += "neuron_ready_" + hex_index + " and "
        ...
        # Multi-input control
        self.architecture.instances.add(self.multi_input, "multi_input_control")
        ...
        for i in range(self.n_neurons):
            ...
            exc_weight_name = "exc_weight_" + int_to_hex(i, hex_width)
            inh_weight_name = "inh_weight_" + int_to_hex(i, hex_width)
            v_th_name = "v_th_" + int_to_hex(i, hex_width)

            if self.lif_neuron.reset == "fixed":
                v_reset_name = "v_reset_" + int_to_hex(i, hex_width)

            neuron_ready_name = "neuron_ready_" + int_to_hex(i, hex_width)

            self.architecture.instances.add(self.lif_neuron, neuron_name)
            ...
            self.architecture.instances[neuron_name].p_map.add("v_th", v_th_name)
        ...
        # Excitatory memory
        self.architecture.instances.add(self.exc_mem, "exc_mem")
        self.architecture.instances["exc_mem"].generic_map()
        self.architecture.instances["exc_mem"].port_map()
        self.architecture.instances["exc_mem"].p_map.add("addra", "exc_addr")
        self.architecture.instances["exc_mem"].p_map.add("clka", "clk")
        for i in range(self.n_neurons):
            exc_weight_name = "exc_weight_" + int_to_hex(i, hex_width)
            dout_name = "dout_" + int_to_hex(i, hex_width)

            self.architecture.instances["exc_mem"].p_map.add(dout_name, exc_weight_name)

        # Excitatory address converter
        self.architecture.instances.add(self.addr_converter, "exc_addr_conv")
        self.architecture.instances["exc_addr_conv"].generic_map()
        self.architecture.instances["exc_addr_conv"].g_map.add("N", "exc_cnt_bitwidth")
        self.architecture.instances["exc_addr_conv"].port_map()
        self.architecture.instances["exc_addr_conv"].p_map.add("addr_in", "exc_cnt")
        self.architecture.instances["exc_addr_conv"].p_map.add("addr_out", "exc_addr")

        # Inhibitory memory
        self.architecture.instances.add(self.inh_mem, "inh_mem")
        self.architecture.instances["inh_mem"].generic_map()
        self.architecture.instances["inh_mem"].port_map()
        self.architecture.instances["inh_mem"].p_map.add("addra", "inh_addr")
        self.architecture.instances["inh_mem"].p_map.add("clka", "clk")
        for i in range(self.n_neurons):
            inh_weight_name = "inh_weight_" + int_to_hex(i, hex_width)
            dout_name = "dout_" + int_to_hex(i, hex_width)

            self.architecture.instances["inh_mem"].p_map.add(dout_name, inh_weight_name)

        # Inhibitory address converter
        self.architecture.instances.add(self.addr_converter, "inh_addr_conv")
        self.architecture.instances["inh_addr_conv"].generic_map()
        self.architecture.instances["inh_addr_conv"].g_map.add("N", "inh_cnt_bitwidth")
        self.architecture.instances["inh_addr_conv"].port_map()
        self.architecture.instances["inh_addr_conv"].p_map.add("addr_in", "inh_cnt")
        self.architecture.instances["inh_addr_conv"].p_map.add("addr_out", "inh_addr")

        # Barrier
        self.architecture.instances.add(self.barrier, "spikes_barrier")
        ...
        self.architecture.instances["spikes_barrier"].p_map.add("reg_out", "out_spikes")
        self.architecture.instances["spikes_barrier"].p_map.add(
            "ready", "barrier_ready"
        )
        ...
        if self.lif_neuron.reset == "none" and self.functional is True:
            self.entity.port.add(
                ...
                direction="out",
                port_type="signed("
                + str(self.bitwidth * self.n_neurons - 1)
                + " downto 0)",
            )
...
class Layer_tb(Testbench):
    ...
    def vhdl(
        ...
        self.architecture.processes["restart_gen"].bodyHeader.add("wait for 10000 ns;")
        ...
        if file_input and "exc_spikes" in input_signal_list:
            ...
            self.architecture.bodyCodeHeader.add("exc_spikes_rd_en <= ready;")

        if file_input and "inh_spikes" in input_signal_list:
            del self.architecture.processes["inh_spikes_rd_en_gen"]
            self.architecture.bodyCodeHeader.add("inh_spikes_rd_en <= ready;")
