...
class MapObj:
    ...
    def code(self, indent_level=0):
        return self.port + self.conn_range + " => " + self.signal + ",\n"
    ...
    def __init__(self, elements_list, mode="auto", *args, **kwargs):
        ...
        if mode == "auto":
            ...
            if len(args) != len(self.elements_list):
                print("Error, wrong number of elements in signals mapping\n")
                exit(-1)
                ...
                for source, target in zip(args, self.elements_list):
                    if isinstance(source, str):
                        ...
                        msg = "Wrong source in signal mapping"
                        raise ValueError(msg)

                    target_name = self.elements_list[target].name

                    self[target_name] = MapObj(target_name, source_name)
            ...
            for target in self.elements_list:
                if self.elements_list[target].name in kwargs:
                    if isinstance(kwargs[target], str):
                        ...
                        source_name = kwargs[target].name
